...
class VelocityEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, xml_file="./models/balance_v1/scene.xml", frame_skip=4, enable_disturbance=False, **kwargs):
        ...
        obs_dim = (
            10 +    # core body + command + wheel states
            4 +     # joint positions
            4 +     # joint velocities
            self.n_actuators
        )
    ...
    def reset_model(self):
        ...
        yaw_mag = 0.7  # rad/s (fixed magnitude)

        sign = self.np_random.choice([-1.0, 1.0])
        self.yaw_rate_cmd = sign * yaw_mag
    ...
    def _get_obs(self):
        ...
        yaw_rate = gyro[2]
        yaw_rate_norm = np.clip(yaw_rate / 3.0, -1.0, 1.0)

        yaw_rate_cmd_norm = np.clip(self.yaw_rate_cmd / 3.0, -1.0, 1.0)    
    ...
    def _compute_reward(self, action, prev_action):
        ...
        # --- Yaw tracking reward ---
        yaw_rate = self.data.sensordata[self.gyro_adr + 2]        # --- Wheel difference shaping for yaw
        yaw_rate_error = yaw_rate - self.yaw_rate_cmd

        yaw_rate_scale = 0.5  # rad/s tolerance
        yaw_rate_reward = np.exp(
            -0.5 * (yaw_rate_error / yaw_rate_scale) ** 2
        )
        
        balance_scale = 1.0 + 1.5 * abs(self.yaw_rate_cmd)
        ...
        ctrl_cost = 0.02 * action[0]**2 + 0.005 * action[1]**2

        # smoothness_cost = 0.01 * np.sum((action - prev_action)**2)
        smoothness_cost = (
            0.01 * (action[0] - prev_action[0])**2 +
            0.003 * (action[1] - prev_action[1])**2
        )
        ...
        reward = (
            ...
            - (2.0 / balance_scale) * theta**2
            - (1.2 / balance_scale) * theta_dot ** 2
            - 0.3 * roll ** 2
            - 0.4 * roll_dot ** 2
